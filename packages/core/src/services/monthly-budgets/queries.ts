...
export class MonthlyBudgetQueries {
  ...
  getTransfersThroughMonth(
    ...
    return allRows<{
      ...
      payee: string;
      sourceAccount: string;
      sourceOnBudget: number;
      destinationAccount: string;
      destinationOnBudget: number;
      excludeFromReadyToAssign: number;
    }>(
      this.db,
      `
      SELECT
        ...
        t.Memo AS memo,
        t.Payee AS payee,
        t.ExcludeFromReadyToAssign AS excludeFromReadyToAssign,
        src.Name AS sourceAccount,
        src.OnBudget AS sourceOnBudget,
        IFNULL(dest.Name, '') AS destinationAccount,
        IFNULL(dest.OnBudget, 0) AS destinationOnBudget
      FROM transactions t
      INNER JOIN accounts src ON t.AccountID = src.ID
      LEFT JOIN transactions mirror ON mirror.TransferID = t.TransferID AND mirror.ID != t.ID
      LEFT JOIN accounts dest ON mirror.AccountID = dest.ID
      WHERE t.BudgetID = ?1
        AND t.TransferID IS NOT NULL
        AND t.TransferID != ''
        AND t.OutflowConverted > 0
    ...
    `,
      ...
      ...r,
      sourceOnBudget: Boolean(r.sourceOnBudget),
      destinationOnBudget: Boolean(r.destinationOnBudget),
      excludeFromReadyToAssign: Boolean(r.excludeFromReadyToAssign),
    }));
  }
