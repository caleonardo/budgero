...
Before saving, the preview compares rows with previously imported data, existing transactions, and other rows in the same file. It labels them **New**, **Already imported**, **Needs review**, or **Invalid/skipped**.

Already-imported rows are skipped by default. For possible matches, choose **Skip** or **Import as new**; import waits until every ambiguous row has a decision. Compare the date, amount, payee, and memo rather than relying only on similar-looking text. You can resolve all remaining rows together or review each one.

If transactions or destination accounts change during review, check the refreshed matches. After importing, reconcile each account against the bank statement (see [Accounts](/docs/accounts)). Non-overlapping date ranges still reduce review work, especially when combining manual entry with bank imports.
