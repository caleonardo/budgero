...
- When a fresh rate is needed, Budgero fetches it from the exchange-rate service and stores it. During the first successful online session of each calendar day, it also refreshes the official rates for the current day so cached rates do not silently remain stale. If you're offline and no cached rate exists, you can supply a manual rate so the conversion can proceed; that manual rate is replaced automatically once you reconnect, unless you turn the "update offline rates" option off — and rates you pin on a transaction are never touched.
