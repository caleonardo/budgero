...
- Body-velden:
  - `encrypted_payload` (string, Base64): met AES-GCM versleutelde JSON-payload (salt + IV + ciphertext + auth-tag; zie [Versleutelingsvereisten](#versleutelingsvereisten)).
...
- IV: 12 bytes (na de salt, vóór de ciphertext)  
- Auth tag: 16 bytes (voeg toe na ciphertext)  
- Codering: Base64 van **salt + IV + ciphertext + tag**  
