...
<Callout title="Support Budgero">
  <p className="!my-0">Self-hosting is free, and donations are optional. If Budgero is useful to you, <TrackedLink href="/donate" event="Self-Host - Donate (Setup Guide)" className="underline underline-offset-4">make a one-time donation</TrackedLink> to help support development.</p>
</Callout>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>public jsDelivr CDN</td>
      <td>Optional: base URL of a self-hosted exchange-rate mirror — see <a href="#currency-conversion">Currency conversion</a></td>
    </tr>
    <tr>
...
Multi-currency conversion works out of the box — no API key, no signup. Rates come from the open [exchange-api](https://github.com/fawazahmed0/exchange-api) dataset (~350 currencies including crypto, updated daily) via the public jsDelivr CDN, and your server caches them locally so repeated requests never leave your machine.
...
The `{date}` placeholder is substituted with the dataset date (YYYY-MM-DD). Single-currency users can ignore all of this — Budgero works fine without it.
...
Changes take effect without restarting the server. When disabled, the login page
hides Sign up, sign-up links redirect to sign-in, and the registration API rejects
new accounts. An already-open page updates on refocus or reload; API enforcement
applies to new registration requests immediately.
