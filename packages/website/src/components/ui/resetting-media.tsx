...
export const ResettingGif = React.forwardRef<
...
>(({ resetKey, src, alt, fill, sizes, priority, unoptimized, className, style, ...props }, ref) => {
  ...
  React.useEffect(() => {
    ...
    const newSrc = `${src}${src.includes('?') ? '&' : '?'}reset=${String(resetKey)}&t=${Date.now()}&r=${Math.random()}`;
  ...
  return (
    <img
      key={`${String(resetKey)}-${mediaUrl}`} // Double key for absolute remount
