...
export default async function FireflyAlternativePage({
  ...
  const t = await getTranslations({
    locale: (await params).locale,
    namespace: 'firefly_iii_alternative',
  });
    ...
    '@graph': [
      {
        ...
        url: 'https://budgero.app/firefly-iii-alternative',
        description: copy('u_c3e7cef077d0'),
        offers: [
          {
            '@type': 'Offer',
          ...
          <div className="relative z-10 px-4 sm:px-6 lg:px-10 xl:px-12 2xl:px-16 py-2 sm:py-4 lg:py-6">
            ...
            <section className="pt-24 pb-16 md:pt-32 md:pb-24 text-center">
              <div className="max-w-4xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  >
                    <Link href="/docs/self-hosting-guide">
                      {' '}
                      {t('self_host_for_free')} <ArrowRight className="w-5 h-5 ml-2" />
                    </Link>
                  </Button>
                  <Button
                    asChild
                ...
                <p className="mt-4 text-sm text-foreground/60">
                  {' '}
                  {t('or_try_budgero_cloud_free_for')}{' '}
                </p>
              </div>
            </section>
            ...
            <section className="py-16 max-w-3xl mx-auto">
              <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-8">
                {' '}
                {t('firefly_iii_is_a_ledger_budgero')}{' '}
              </h2>
              <div className="space-y-6 text-lg text-foreground/75 leading-relaxed">
                <p> {t('let_s_be_fair_to_firefly')} </p>
                <p>
                  {' '}
                  {t('the_reason_people_go_looking_for')} <em>{t('next')}</em>
                  {t('every_unit_of_money_gets_a')} <em>{copy('u_fa51fd49abf6')}</em>{' '}
                  {t('the_core_the_same_philosophy_that')}{' '}
                  <Link
            ...
            <section id="comparison" className="py-16 max-w-5xl mx-auto">
              <div className="text-center mb-12">
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
                  {' '}
                  {t('budgero_self_host_vs_firefly_iii')}{' '}
                </h2>
              </div>
              ...
              <p className="mt-6 text-foreground/60 text-sm max-w-3xl">
                <strong className="text-foreground">{t('key_takeaway')}</strong>{' '}
                {t('pick_firefly_iii_for_an_open')}{' '}
              </p>
            </section>
            ...
            <section className="py-20 text-center">
              <div className="max-w-2xl mx-auto">
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-6">
                  {' '}
                  {t('your_server_your_data_a_budget')}{' '}
                </h2>
                <p className="text-lg text-foreground/70 mb-8">
                  {' '}
                  {t('spin_up_budgero_self_host_with')}{' '}
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button
                    asChild
                  ...
                  >
                    <Link href="/docs/self-hosting-guide">
                      {' '}
                      {t('read_the_self_hosting_guide')} <ArrowRight className="w-5 h-5 ml-2" />
                    </Link>
                  </Button>
                  <Button
                    asChild
