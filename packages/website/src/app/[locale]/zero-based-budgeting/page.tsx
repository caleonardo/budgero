...
const makeSteps = (t: (key: string, values?: Record<string, string | number>) => string) => [
  {
    ...
    description: t('steps_start_with_what_you_will_earn'),
  },
  {
    number: '2',
    title: t('steps_list_your_expenses'),
    description: t('steps_fixed_costs_rent_utilities_insurance_variable'),
  },
  {
    number: '3',
    title: t('steps_assign_every_dollar'),
    description: t('steps_distribute_your_income_across_categories_until'),
  },
  {
    number: '4',
    title: t('steps_track_and_adjust'),
    description: t('steps_as_the_month_progresses_track_actual'),
  },
];
...
export default async function ZeroBasedBudgetingPage({
  ...
  const t = await getTranslations({
    locale: (await params).locale,
    namespace: 'zero_based_budgeting',
  });
  ...
  const jsonLd = {
    ...
    description: copy('u_55c5859525ab'),
  };
          ...
          <div className="relative z-10 px-4 sm:px-6 lg:px-10 xl:px-12 2xl:px-16 py-2 sm:py-4 lg:py-6">
            ...
            <section className="py-16 max-w-4xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {steps.map((step) => (
                  <div key={step.number} className="bg-card rounded-xl p-6 border border-border/70">
            ...
            <section className="py-16 max-w-4xl mx-auto">
              <div className="text-center mb-12">
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">
                  {' '}
                  {t('zero_based_vs_other_methods')}{' '}
                </h2>
              </div>

              <div className="grid md:grid-cols-3 gap-6">
                {methods.map((method) => (
                  <div key={method.name} className="bg-card rounded-xl p-6 border border-border/70">
                    ...
                    <p className="text-xs text-foreground/55">
                      <strong className="text-foreground/70">{t('best_for')}</strong>{' '}
                      {method.bestFor}
                    </p>
                  </div>
                ))}
              ...
              <p className="mt-8 text-center text-foreground/60">
                {' '}
                {t('zero_based_budgeting_requires_the_most')}{' '}
              </p>
            </section>
            ...
            <section className="py-16 max-w-3xl mx-auto">
              <div className="bg-card rounded-2xl p-8 border border-border/70">
                <p className="text-foreground/75 mb-6">
                  {' '}
                  {t('if_you_want_a_zero_based')} {pricing.yearly}
                  {t('yr_budgero_might_be_a_good')}{' '}
                </p>
                <Button
                  asChild
                  size="lg"
